...
class BoardSettingsViewModel: ObservableObject {
    ...
    func getAuthorizedUsers(from boardID: String) {
        db.collection("boards").document(boardID).addSnapshotListener { (querySnapshot, error) in
...
//        })
    ...
    func removeUserFromBoard(at indices: IndexSet, boardID: String) {
        indices.sorted(by: > ).forEach { index in
            ...
            db.collection("boards").document(boardID).updateData(["users": FieldValue.arrayRemove([removedUser.id])], completion: { error in
                guard error == nil else {
                    return
                }
                print("passed error check")
                print(self.editors)
//                self.editors.remove(at: index)
                print(self.editors)
            })
        }
    }
